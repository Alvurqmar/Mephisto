...
export function StewE(gameState: GameState, cardId: string) {
  ...
  if (!position || !card) {
    console.warn("Stew card not found on field");
    return gameState;
  }
  ...
  if (orientation === 'horizontal') {
    for (let col = 0; col < gameState.field.slots[Row].length; col++) {
      ...
      if (slot.card && slot.card.type === "MONSTER" && slot.card.id !== numericCardId) {
        bonusFP += 2;
      }
    }
  } else {
